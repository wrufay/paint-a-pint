...
export const BOX = PALETTES["my box"].map((id) =>
  PAINTS.find((p) => p.id === id)
);
...
export class AcrylicPainter {
  constructor(canvas, grainCanvas, w = 1300, h = 950) {
    ...
    this.albedoCanvas.width = w;
    this.albedoCanvas.height = h;
  ...
  get size() {
    return this.engine.params.size;
  }
  set size(v) {
    this.engine.params.size = v;
  }

  setPaint(paint) {
    this.paint = paint;
    this.color = hexToLinear(paint.hex);
    this.tool = "brush";
  }
  setMixedColour(rgb, paint) {
    this.paint = paint;
    this.color = rgb;
    this.tool = "brush";
  } // a colour mixed on the palette, standing in for a tube
  clear() {
  ...
  undo() {
    if (this.engine.restore()) this._blit(false);
  }
  dryNow() {
    this.engine.snapshot();
    this.engine.dryAll();
  ...
  setWetnessView(on) {
    this.engine.setDebugWet(on);
    this._blit(true);
  } // blue = still workable, orange = tacky, none = dry
  savePng() {
    this.composite().toBlob((blob) => {
      if (!blob) return;
      const a = document.createElement("a");
      a.download = "paint-a-pint.png";
  ...
  down(x, y, pressure = 0.5, isPen = false) {
    ...
    this.engine.beginStroke(
      x,
      y,
      this._pressure(x, y, pressure, isPen),
      this.color,
      this.paint
    );
    this.last = [x, y];
  ...
  up() {
    if (this.drawing) this.engine.endStroke();
    this.drawing = false;
  }
  ...
  _blit(all) {
    ...
    if (!m) this.mapRect = { ...r };
      ...
      const x1 = Math.max(m.x + m.w, r.x + r.w),
        y1 = Math.max(m.y + m.h, r.y + r.h);
  ...
  syncMaps() {
    ...
    this.albedoCanvas
      .getContext("2d")
      .putImageData(this.albedoImg, 0, 0, m.x, m.y, m.w, m.h);
    this.normalCanvas
      .getContext("2d")
      .putImageData(this.normalImg, 0, 0, m.x, m.y, m.w, m.h);
    this.mapRect = null;
    return true;
  }
...
export function initAcrylicUI(
  painter,
  { onBack, view, isPainting = () => true }
) {
  ...
  let chipEl = null,
    brushName = null; // the brush-colour chip and its label
  let choose = (p) => {
    painter.setPaint(p);
    sync();
  }; // replaced below once the tray tools exist
  const cssColour = (rgb) =>
    "#" +
    rgb
      .map((v) =>
        Math.round(Math.min(1, linearToSrgb(Math.max(0, v))) * 255)
          .toString(16)
          .padStart(2, "0")
      )
      .join("");
  const sync = () => {
    swatches
      .querySelectorAll(".sw")
      .forEach((b) =>
        b.classList.toggle("sel", b.dataset.id === painter.paint.id)
      );
    if (chipEl) chipEl.style.background = cssColour(painter.color);
    if (brushName)
      brushName.textContent =
        painter.paint.id === "mixed" ? "your own mix" : painter.paint.name;
    if (ui.onSelect) ui.onSelect(painter.paint);
  };
  BOX.forEach((p, i) => {
    const b = document.createElement("button");
    const name = `${i + 1} · ${p.brand} · ${p.name}${
      p.code ? " · " + p.code : ""
    }`; // the number is the key that picks it
    b.className = "sw";
    b.title = name;
  ...
  size.min = 10;
  size.max = 140;
  size.value = painter.size;
  size.oninput = () => {
    painter.size = +size.value;
    updateCursor();
  };
  ...
  const syncShape = () => {
    for (const k in shapeBtns)
      shapeBtns[k].classList.toggle("sel", painter.engine.params.shape === k);
    if (ui.onShape) ui.onShape(painter.engine.params.shape);
  };
  for (const k of ["flat", "filbert", "round", "knife"]) {
    ...
    b.textContent = k;
    b.title = {
      flat: "flat brush: a row of bristles",
      filbert: "filbert: an oval tip, width follows pressure",
      round: "round brush: click for a dab",
      knife: "palette knife: drags paint into flat planes with a raised edge",
    }[k];
    b.onclick = () => {
      painter.engine.params.shape = k;
      saveParams(painter.engine.params);
      syncShape();
    };
    shapeBtns[k] = b;
  ...
  if (view) {
    ...
    const syncView = () => {
      viewBtn.textContent = view.down
        ? "change view to easel"
        : "change view to birds-eye";
    };
    viewBtn.onclick = () => view.set(!view.down);
    view.onChange = syncView;
    syncView();
  ...
  const panel = buildTunePanel(painter.engine.params, DEFAULTS, {
    onChange: () => {
      size.value = painter.size;
      updateCursor();
    },
  ...
  let wet = false;
  const wetBtn = mk("wetness", () => {
    wet = !wet;
    painter.setWetnessView(wet);
    wetBtn.classList.toggle("sel", wet);
    wetHint.style.display = wet ? "block" : "none";
  });
  mk("save png", () => painter.savePng());
  const settingsBtn = mk("settings", () => {
    const on = panel.style.display === "none";
    panel.style.display = on ? "block" : "none";
    settingsBtn.classList.toggle("sel", on);
    if (on) panel.refresh();
  });
  const wetHint = document.createElement("p");
  wetHint.textContent =
    "blue = still workable · orange = getting tacky · no tint = dry";
  wetHint.style.cssText =
    "display:none;margin:0 0 12px;font-size:var(--text-xs);color:var(--ink-soft);text-align:center;";
  ...
  chipEl.style.cssText =
    "flex:none;width:28px;height:28px;border-radius:50%;box-shadow:0 0 0 2px var(--ink);";
  brushName = document.createElement("div");
  brushName.style.cssText =
    "font-size:var(--text-sm);line-height:1.25;color:var(--ink);";
  brushRow.append(chipEl, brushName);
  swatches.before(brushRow);

  let trayMode = "squeeze",
    squeezePaint = painter.paint;
  const trayBox = document.createElement("div");
  trayBox.style.cssText = "margin:0 0 12px;";
  const trayLabel = document.createElement("div");
  trayLabel.textContent = "palette tray";
  trayLabel.style.cssText =
    "font-size:var(--text-xs);letter-spacing:var(--tracking-caps);text-transform:uppercase;color:var(--ink-soft);margin:0 0 6px;";
  const trayRow = document.createElement("div");
  trayRow.className = "card-tools";
  trayRow.style.cssText = "margin:0 0 6px;";
  const trayBtns = {};
  const trayHint = document.createElement("p");
  trayHint.style.cssText =
    "margin:0;font-size:var(--text-xs);line-height:1.4;color:var(--ink-soft);";
  const syncTray = () => {
    for (const k in trayBtns)
      trayBtns[k].classList.toggle("sel", k === trayMode);
    trayHint.textContent =
      trayMode === "squeeze"
        ? `tap or drag on the tray to squeeze out ${
            squeezePaint ? squeezePaint.name : "paint"
          }. Click another tube to switch.`
  ...
  for (const [k, label] of [
    ...
    trayBtns[k] = b;
    trayRow.appendChild(b);
  }
  const wipeBtn = document.createElement("button");
  wipeBtn.className = "btn";
  wipeBtn.textContent = "wipe";
  wipeBtn.title = "scrape the tray clean";
  ...
  ui.setDesk = (down) => {
    swatches.style.display = down ? "none" : "";
    trayBox.style.display = down ? "" : "none";
    if (ui.setEaselOnly) ui.setEaselOnly(!down);   // the easel view is view-only (defined below, once the card exists)
  };
  ui.setDesk(view ? view.down : false);
  ui.trayMode = () => trayMode;
  ui.squeezePaint = () => squeezePaint;
  ui.loadMixed = (rgb, paint) => {
    painter.setMixedColour(rgb, paint);
    sync();
  };
  ...
  // ── the easel view is view-only ───────────────────────────────────────────────────────────────────────────────────────────
  // With the canvas standing on the easel you look at it: no paper to paint on, no card, no settings, and the paint keys do nothing.
  // The only thing left is a small button to go back to the desk, where you paint.
  let easelOnly = false;
  const backBtn = document.createElement("button");
  backBtn.className = "btn";
  backBtn.textContent = "change view to birds-eye";
  backBtn.style.cssText =
    "display:none;position:absolute;left:50%;bottom:26px;transform:translateX(-50%);width:auto;margin:0;padding:9px 20px;box-shadow:0 4px 14px rgba(0,0,0,.3);";
  backBtn.onclick = () => view && view.set(true);
  document.getElementById("paint").appendChild(backBtn);
  ui.setEaselOnly = (on) => {
    easelOnly = on;
    paper.style.display = on ? "none" : "";
    document.getElementById("card").style.display = on ? "none" : "";
    if (on) {
  ...
  ui.setEaselOnly(view ? !view.down : false);

  // ── drag the card by its heading (double-click the heading to put it back) ────────────────────────────────────────────
  const card = document.getElementById("card"),
    handle = card.querySelector("h2"),
    POS_KEY = "paint-a-pint:card-pos";
  ...
  const place = (x, y) => {
    ...
    y = Math.min(Math.max(0, y), Math.max(0, innerHeight - r.height));
  ...
  const home = () => {
    for (const k of ["left", "top", "right", "bottom", "transform"])
      card.style[k] = "";
    try {
  ...
  handle.addEventListener("pointermove", (e) => {
    if (cardDrag && e.pointerId === cardDrag.id)
      place(e.clientX - cardDrag.dx, e.clientY - cardDrag.dy);
  });
  const cardDrop = (e) => {
    ...
    try {
      localStorage.setItem(
        POS_KEY,
        JSON.stringify({ x: r.left / innerWidth, y: r.top / innerHeight })
      );
    } catch {}
  };
  ...
  try {
    ...
    if (saved) place(saved.x * innerWidth, saved.y * innerHeight);
  } catch {}
  ...
  if (esc)
    esc.textContent = "esc: back to the room, painting stays on the easel";
  sync();

  // keyboard: 1-9 pick a paint (card order), [ ] brush size, z undo, w wetness. Only while painting, and never while typing.
  addEventListener("keydown", (e) => {
    if (!isPainting() || easelOnly || e.ctrlKey || e.metaKey || e.altKey) return;
    const t = e.target;
    if (
      t &&
      (t.isContentEditable ||
    ...
    if (k >= "1" && k <= "9") {
      ...
      painter.size = Math.round(
        Math.min(
          +size.max,
          Math.max(+size.min, painter.size * (k === "]" ? 1.15 : 1 / 1.15))
        )
      );
      size.value = painter.size;
      updateCursor();
      if (panel.style.display !== "none") panel.refresh();
  ...
  const toCanvas = (e) => {
    ...
    return [
      ((e.clientX - r.left) / r.width) * painter.canvas.width,
      ((e.clientY - r.top) / r.height) * painter.canvas.height,
      r.width / painter.canvas.width,
    ];
  };
  let cursorScale = 0.5;
  const updateCursor = () => {
  ...
  paper.addEventListener("pointerdown", (e) => {
    ...
    else if (e.pointerType === "touch" && performance.now() - penSeen < 8000)
      return; // palm rejection once the Pencil is in use
    if (active !== null) return;
    active = e.pointerId;
    paper.setPointerCapture(e.pointerId);
    const [x, y, s] = toCanvas(e);
    cursorScale = s;
    updateCursor();
  ...
  paper.addEventListener("pointermove", (e) => {
    ...
    for (const ev of evs.length ? evs : [e]) {
      ...
      painter.move(x, y, ev.pressure || 0.5);
    }
  });
  const end = (e) => {
    if (e.pointerId !== active) return;
  ...
  paper.addEventListener("pointerenter", () => {
    cursor.style.opacity = 1;
    updateCursor();
  });
  return ui;
